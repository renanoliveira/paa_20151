...
import csv
import time
import dijkstra_buckets2


if __name__ == "__main__":
	# Importar dados de instancias
	files = os.listdir("instancias/fibonacci")

	with open('fibonacci.csv', 'w') as csvfile:
		fieldnames = ['instancia', 'nodes', 'edges', 'tempo_execucao']
		writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
		writer.writeheader()
		for filename in files:
			g = dijkstra_fibonacci.Graph()

			f = open("instancias/fibonacci/" + filename,'r')
			f_words = f.read()
			...
			while True:
				...
				dijkstra_fibonacci.dijkstra(g, g.get_vertex(1))
