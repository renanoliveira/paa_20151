...
"""
    ...
    def insert(self, t):
        ...
        if self.root is None:
            ...
            node = self.root
            while True:
                if t[0] < node.key or (t[0] == node.key and t[1] < node.label):
                    # Go left
    ...
    def find(self, t_key, t_label):
        """Return the node for key t if is in the tree, or None otherwise."""
        node = self.root
        while node is not None:
            if t_key == node.key and t_label == node.label:
                return node
            elif t_key < node.key or (t_key == node.key and t_label < node.label):
                node = node.left
            else:
                node = node.right
        return None
    ...
    def __str__(self):
        ...
        def recurse(node):
            ...
            label = str(node.key) + ' ' + str(node.label)
